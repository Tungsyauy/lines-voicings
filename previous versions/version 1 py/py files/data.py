...
def get_7sus4_chord_display(key, chord_type):
    """Generate chord display based on selected key and chord type"""
    if chord_type == "minor":
        return f"in the key of {key}m"
    elif chord_type == "dominant":
        return f"in the key of {key}7"
    elif chord_type == "half_dim":
        return f"in the key of {key}ø7"
    elif chord_type == "altered":
        return f"in the key of {key}7Alt"
    
    return f"in the key of {key}"

def find_7sus4_generation_key(target_key, chord_type):
    """Find which key's 7sus4 phrase should be used to generate phrases for the target chord"""
    # Create the target chord string
    if chord_type == "minor":
        target_chord = f"{target_key}m"
    elif chord_type == "dominant":
        target_chord = f"{target_key}7"
    elif chord_type == "half_dim":
        target_chord = f"{target_key}ø7"
    elif chord_type == "altered":
        target_chord = f"{target_key}7Alt"
    else:
        return target_key
    
    # Search through all 7sus4 mappings to find which key's phrase is usable on target_chord
    for generation_key, chord_text in KEY_CHORD_MAP["7sus4"].items():
        if "usable on " in chord_text:
            chords = chord_text.replace("usable on ", "").split(", ")
            if target_chord in chords:
                return generation_key
    
    # If not found, return the original key
    return target_key

def find_7sus4_target_key_for_display(generation_key, chord_type):
    ...
    if generation_key not in KEY_CHORD_MAP["7sus4"]:
        return generation_key
    
    chord_text = KEY_CHORD_MAP["7sus4"][generation_key]
    if "usable on " not in chord_text:
        return generation_key
    ...
    for chord in chords:
        if chord_type == "minor" and chord.endswith("m"):
            return chord[:-1]  # Remove 'm' suffix
        elif chord_type == "dominant" and chord.endswith("7") and not chord.endswith("ø7") and not chord.endswith("Alt"):
            return chord[:-1]  # Remove '7' suffix
        elif chord_type == "half_dim" and chord.endswith("ø7"):
            return chord[:-2]  # Remove 'ø7' suffix
        elif chord_type == "altered" and chord.endswith("Alt"):
            return chord[:-4]  # Remove '7Alt' suffix
    
    # If no match found, return the generation key
    return generation_key

# Add new mappings for specific 7sus4 chord types
KEY_CHORD_MAP["7sus4_minor"] = {}
KEY_CHORD_MAP["7sus4_dominant"] = {}
KEY_CHORD_MAP["7sus4_half_dim"] = {}
KEY_CHORD_MAP["7sus4_altered"] = {}

# Populate the new mappings
for key in KEY_CHORD_MAP["7sus4"].keys():
    KEY_CHORD_MAP["7sus4_minor"][key] = get_7sus4_chord_display(key, "minor")
    KEY_CHORD_MAP["7sus4_dominant"][key] = get_7sus4_chord_display(key, "dominant")
    KEY_CHORD_MAP["7sus4_half_dim"][key] = get_7sus4_chord_display(key, "half_dim")
    KEY_CHORD_MAP["7sus4_altered"][key] = get_7sus4_chord_display(key, "altered")